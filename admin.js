...
async function updateTournamentPlayerRanks(tournamentId) {
    try {
      ...
      // Update each player's tournament_rank
      for (let i = 0; i < sortedPlayers.length; i++) {
        ...
        const { error: updateError } = await supabase
          .from('tournament_players')
          .update({ tournament_rank: tournamentRank })
      ...
      showToast(`Updated tournament ranks for ${sortedPlayers.length} players`, 'success');
      return true;
    } catch (err) {
...
function parsePlayerImportData() {
    ...
    try {
        ...
        // Process players with enhanced error logging
        const parsedPlayers = playersData.map(player => {
            ...
            return {
                acl_player_id: player.playerID || player.acl_player_id || null,
                name: player.fldPlayerFirstName && player.fldPlayerLastname ? 
                    `${player.fldPlayerFirstName} ${player.fldPlayerLastname}` : 
                    player.name || "Unknown Player",
                skill_level: player.playerSkillLevel || player.skill_level || 'P',
                player_ppr: parseFloat(player.playerPPR || player.player_ppr) || 0,
                player_cpi: parseFloat(player.playerCPI || player.player_cpi) || 0,
                rank: temporaryRank,
                // Calculate potential points based on rank
                potential_points: calculatePotentialPoints(temporaryRank),
                // Initialize with zero actual points
                actual_points: 0,
...
async function checkExistingPlayersGlobal(players) {
    try {
        ...
        if (aclPlayerIds.length > 0) {
            // Query existing players by ACL ID
        ...
        if (playerNames.length > 0) {
            ...
            for (let i = 0; i < playerNames.length; i += batchSize) {
                ...
                let query = supabase.from('players').select('id, name, acl_player_id');
        ...
        // Update player status based on matches
        players.forEach(player => {
            ...
            // Then check by name (for players without ACL ID)
                ...
                player.id = existingPlayerMap[player.name.toLowerCase()].id;
            }
...
function displayParsedPlayersGlobal(players) {
    ...
    summary.innerHTML = `
        ...
        <span class="status-existing">${stats.existing} existing</span>
    `;
    ...
    players.forEach(player => {
        ...
        row.innerHTML = `
            ...
            <td>${player.player_ppr !== null && player.player_ppr !== undefined ? player.player_ppr : 'N/A'}</td>
            <td>${player.player_cpi !== null && player.player_cpi !== undefined ? player.player_cpi : 'N/A'}</td>
...
async function importPlayersGlobal() {
    ...
    try {
        ...
        // Update rankings after adding players
        await updatePlayerRanksByCPI();
        ...
        showToast(`Import complete: ${successCount} added, ${skipCount} skipped, ${failCount} failed`);
