...
async function loadTeams() {
    try {
        ...
        for (const team of teams) {
            ...
            // Get tournament result for this team
            let placementInfo = 'Not Ranked';
            ...
            if (tournamentResult && tournamentResult.placement > 0) {
                const placements = ['', '1st', '2nd', '3rd', '4th'];
                placementInfo = `${placements[tournamentResult.placement]} (${tournamentResult.points_earned} pts)`;
            }
...
async function viewTeamDetails(teamId) {
    try {
        ...
        if (tournamentResult && tournamentResult.placement > 0) {
            const placements = ['', '1st Place (100%)', '2nd Place (75%)', '3rd Place (50%)', '4th Place (25%)'];
            resultInfo = `
                <div class="result-info">
                    <p><strong>Team Placement:</strong> ${placements[tournamentResult.placement]}</p>
                    <p><strong>Team Points:</strong> ${tournamentResult.points_earned}</p>
                </div>
            `;
        }
        ...
        if (teamPlayers && teamPlayers.length > 0) {
            ...
            for (const tp of teamPlayers) {
                ...
                if (player) {
                    // Get player's tournament points specifically for this tournament
                    let playerTournamentPoints = null;
                    ...
                    if (pointsRecord) {
                        playerTournamentPoints = pointsRecord.points_earned;
                    }
                    
                    // Calculate points earned for this player based on placement
                    let pointsEarned = 0;
                    let pointsInfo = '';
                    
                    if (tournamentResult && tournamentResult.placement > 0) {
                        const percentages = [0, 1.0, 0.75, 0.5, 0.25]; // Index matches placement
                        const percentage = percentages[tournamentResult.placement];
                        pointsEarned = Math.round(tp.potential_points * percentage);
                        
                        // Use the recorded points if available, otherwise use calculated points
                        if (playerTournamentPoints !== null) {
                            pointsEarned = playerTournamentPoints;
                        }
                        
                        pointsInfo = `
                            <div style="display: flex; justify-content: space-between; font-size: 0.9rem; margin-top: 8px;">
                                <div>Potential: ${tp.potential_points}</div>
                                <div>Earned in Tournament: ${pointsEarned}</div>
                            </div>
                            <div style="font-size: 0.8rem; color: rgba(255,255,255,0.7); text-align: right;">
                                ${(percentage * 100)}% of potential points
                            </div>
                        ...
                        pointsInfo = `
                            <div style="font-size: 0.9rem; margin-top: 8px;">
                                Potential Points: ${tp.potential_points}
                            </div>
                            <div style="font-size: 0.9rem; margin-top: 8px; color: #4ade80;">
                                <strong>Total Career Points: ${player.actual_points || 0}</strong>
                            </div>
